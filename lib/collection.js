...
/**
 * TODO: rename arguments to operator meaningful names
 */
updateOperators.$inc = function(doc, field, value) {
	if (!utils.isNumber(doc[field])) throw new Error(
		'Cannot apply $inc modifier to non-number'
	);
	if (!utils.isNumber(value)) throw new Error(
		'Modifier $inc allowed for numbers only'
	);
	doc[field] = doc[field] + value;
};
updateOperators.$unset = function(doc, field, value) {
	delete doc[field];
};
updateOperators.$rename = function(doc, field, value) {
	if (field in doc) {
		doc[value] = doc[field];
		delete doc[field];
	}
...
/**
 * TODO: add support for modifiers like `$each`, `$sort', `$slice'
 */
updateOperators.$push = function(doc, field, value) {
	if (field in doc === false) doc[field] = [];
	if (!utils.isArray(doc[field])) throw new Error(
		'Cannot apply $push/$pushAll modifier to non-array'
	);
	doc[field].push(value);
};
updateOperators.$addToSet = function(doc, field, value) {
	if (doc[field] && doc[field].indexOf && doc[field].indexOf(value) == -1) {
		updateOperators.$push(doc, field, value);
	}
};
updateOperators.$pushAll = function(doc, field, value) {
	if (field in doc === false) doc[field] = [];
	if (!utils.isArray(doc[field])) throw new Error(
		'Cannot apply $push/$pushAll modifier to non-array'
	);
	if (!utils.isArray(value)) throw new Error(
		'Modifier $pushAll/pullAll allowed for arrays only'
	);
	doc[field] = doc[field].concat(value);
};
/**
 * TODO: add query support (not just equal)
 */
updateOperators.$pull = function(doc, field, value) {
	if (!utils.isArray(doc[field])) throw new Error(
		'Cannot apply $pull/$pullAll modifier to non-array'
	);
	do {
		var index = doc[field].indexOf(value);
		if (index != -1) doc[field].splice(index, 1);
	} while (index != -1);
};
updateOperators.$pullAll = function(doc, field, value) {
	if (!utils.isArray(doc[field])) throw new Error(
		'Cannot apply $pull/$pullAll modifier to non-array'
	);
	if (!utils.isArray(value)) throw new Error(
	...
	value.forEach(function(value) {
		updateOperators.$pull(doc, field, value);
	});
};
...
Collection.prototype.update = function(query, modifier) {
	...
	this.find(query).toArray(function(err, docs) {
		...
		if (docs.length === 0) {
			if (options.upsert) {
				if (isWholeDocUpdate) {
					// unlink modifie from source like mongodb does
					self.insert(utils.clone(modifier), callback);
				} else {
					callback(new Error(
