...
/**
 * Patches methods for waiting collection load
 */
['insert', 'update', 'remove', 'find', 'findOne'].forEach(function(method) {
	var oldMethod = Collection.prototype[method];
	Collection.prototype[method] = function() {
		var self = this;
		if (self._loaded.err) {
			// TODO: pass error to callback instead
			throw self._loaded.err;
			return;
		}
		if (self._loaded && self._loaded.value) {
			return oldMethod.apply(self, arguments);
		} else {
			setTimeout(function() {
				Collection.prototype[method].apply(self, arguments);
			}, 100);
		}
	};
});

Collection.prototype._setLoaded = function(value, err) {
	this._loaded = {value: value, err: err};
};
